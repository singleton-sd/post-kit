...
React admin editor component for [PostKit](../../README.md) email templates. Once complete it will edit the three Git-backed source files (`template.json`, `metadata.json`, `preview.json`) in memory and hand them back to the host application to persist.

This package is currently a **scaffold**: it exports the public surface (`EmailTemplateEditor`, `EmailTemplateEditorProps`, `TemplateSourceFiles`) and renders an empty root container. It does not yet edit the template or invoke `onSave` / `onSendTest`. The canvas, metadata form, preview pane, validation surfaces, and the callback wiring all arrive in later issues.
...
The editor never writes to disk, Git, or a network endpoint. It is designed to call `onSave` with the edited `TemplateSourceFiles` and let the host decide how to commit them; `onSendTest` is likewise a host-supplied hook. Neither callback is invoked by the current scaffold — the props exist so consumers and later issues can rely on a stable contract.
