...
| 503 | `PROVIDER_FAILURE` | Intermittent; `failureClass=transient` with an HTTP `statusCode` (5xx/408/409) — clear provider reject/throttle | Provider outage or throttling after a status response. With an `Idempotency-Key`, the send path may already have retried internally (bounded attempts + backoff) | Provider status; whether `durationMs` / `attempt` are near the timeout ceiling; see [`send-timeout-retry.md`](../architecture/send-timeout-retry.md) | Yes — retry with the **same** `Idempotency-Key` (application replay / re-claim; not provider-side dedupe) |
| 503 | `PROVIDER_FAILURE` | Intermittent; `failureCategory=timeout` or transport failure **without** `statusCode` | Request timed out or the connection dropped after the POST may already have left the client — ambiguous whether Forward Email accepted | Provider status; treat as at-least-once until a provider idempotency mechanism exists | **Caution** — same key is still recommended so a *completed* ledger entry is not re-sent, but a second delivery is possible if the first POST was accepted |
